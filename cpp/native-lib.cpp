...
Java_com_example_securestorage2_10_Storage_Crypt_1AES(JNIEnv *env, jobject thiz, jstring path,
                                                      jboolean flag) {
    ...
    unsigned char key[] = {0x2b, 0x7e, 0x15, 0x16,
                           ...
                           0xab, 0xf7, 0x15, 0x88,
                           0x09, 0xcf, 0x4f, 0x3c};
    char* path_to_file = const_cast<char *>(env->GetStringUTFChars(path,0));
    ...
    AES aes(key);
    FILE *outFile = fopen(path_to_file,"wb+");

    if(outFile==NULL)
        return false;
    if(flag==true) {
        aes.Cipher(buff);//шифрование
    }
    else
    {
        aes.InvCipher(buff);//расшифрование
    }
    for(int i=0;i<strlen(reinterpret_cast<const char *const>(buff)); i++)
    {
        fprintf(outFile,"%c",buff[i]);
    }
    fclose(outFile);
    return true;
}
